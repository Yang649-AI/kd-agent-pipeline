...
def load_ocr_cache(paths: List[Path]) -> Dict[Tuple[str, int], str]:
    ...
    for path in paths:
        ...
        for item in load_jsonl(path):
            ...
            if page > 0:
                # Some OCR caches use 1-based page numbers, while parsed_pages.jsonl uses 0-based page indexes.
...
def page_record_with_ocr_text(page_record: Dict, text: str, parse_method: str) -> Dict:
    ...
    updated["originally_scanned_or_image_page"] = bool(page_record.get("is_scanned_or_image_page", False))
    return updated
...
def build_chunks_for_page(page_record: Dict) -> List[Dict]:
    ...
    for idx, chunk_text in enumerate(chunks):
        ...
        output.append({
            ...
            "originally_scanned_or_image_page": bool(page_record.get("originally_scanned_or_image_page", False)),
    ...
    with CHUNK_OUTPUT_PATH.open("w", encoding="utf-8") as chunk_out, \
        ...
        for page_record in load_jsonl(INPUT_PATH):
            ...
            if page_record.get("needs_ocr"):
                ...
                if cached_text:
                    page_record = page_record_with_ocr_text(page_record, cached_text, "tesseract_ocr_cached")
                    ocr_chunked_pages += 1
                elif ENABLE_OCR and (OCR_MAX_PAGES <= 0 or ocr_runtime_pages < OCR_MAX_PAGES):
                    runtime_text = run_tesseract_ocr(page_record)
                    if runtime_text:
                        page_record = page_record_with_ocr_text(page_record, runtime_text, "tesseract_ocr")
